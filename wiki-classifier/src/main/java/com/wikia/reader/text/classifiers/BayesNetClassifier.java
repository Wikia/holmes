...
public class BayesNetClassifier implements Classifier {
    private static Logger logger = LoggerFactory.getLogger(BayesNetClassifier.class);
    private final List<InstanceSource> sources;
    private Filter<Collection<InstanceSource>, Instances> instancesBuilder;
    private BayesNet bayesNet;
    private List<String> classes;

    public BayesNetClassifier(List<InstanceSource> sources, List<String> classes) {
    ...
    public Classification classify(InstanceSource source) {
        ...
        try {
            double[] doubles = bayesNet.distributionForInstance(instance);
    ...
    protected void train(List<InstanceSource> sources) {
        ...
        try {
            ...
            bayesNet.buildClassifier(instancesBuilder.filter(sources));
            ArffUtil.save(instances, "bayesNet.arff");
        } catch (Exception e) {
