...
public class KStarClassifier implements Classifier {
    ...
    public KStarClassifier(List<InstanceSource> sources, List<String> classes) {
    ...
    public Classification classify(InstanceSource source) {
        ...
        try {
            double[] doubles = kStar.distributionForInstance(instance);
    ...
    protected void train(List<InstanceSource> sources) {
        ...
        try {
            ...
            kStar.buildClassifier(instancesBuilder.filter(sources));
            ArffUtil.save(instances, "kstar_dump.arff");
        } catch (Exception e) {
